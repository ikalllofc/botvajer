...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
    ...
    pemuda.browserDescription = ['Herman Botz','Desktop','3.0']
    await sleep(10000)
    pemuda.on('qr', qr => {
 ...
 teks = `https://chat.whatsapp.com/DHKyawJfcH74DUJkJvzGDH`  //JANGAN DI GANTI BEGO ERROR NANTI NANGES
 teks = `https://chat.whatsapp.com/DHKyawJfcH74DUJkJvzGDH`  //JANGAN DI GANTI BEGO ERROR NANTI NANGES
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   pemuda.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6283146208804@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Subcribe : Herman Chanel', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('By Denis Putra - Recode by Herman Botz' ,{ flashStyle: term.brightWhite })
